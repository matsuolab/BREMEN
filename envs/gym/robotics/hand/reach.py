...
from envs.gym.robotics import hand_env
from envs.gym.robotics.utils import robot_get_obs
...
class HandReachEnv(hand_env.HandEnv, utils.EzPickle):
    ...
    def _get_obs(self):
        ...
        return np.concatenate([observation.copy(), self.goal.copy()])
    ...
    def step(self, action):
        ...
        info = {
            'is_success': self._is_success(obs[-30:-15], self.goal),
        }
        reward = self.compute_reward(obs[-30:-15], self.goal, info)
        return obs, reward, done, info

    def cost_np_vec(self, obs, acts, next_obs):
        reward = -np.linalg.norm(next_obs[:, -30:-15] - next_obs[:, -15:], axis=-1)
        return -reward
